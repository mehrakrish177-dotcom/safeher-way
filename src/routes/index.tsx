...
import { MapPin, PhoneCall, Route as RouteIcon, ShieldCheck, Radio, Clock } from "lucide-react";
...
function Dashboard() {
  const { sharingLocation, setSharingLocation, contacts, events, logEvent, recording } = useSafeHer();
    ...
    <div className="space-y-7">
      <section className="rounded-3xl border border-border bg-card p-5 shadow-soft">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3 sm:flex sm:justify-between">
          ...
          <Badge
            className="shrink-0"
            variant={sharingLocation || recording ? "destructive" : "secondary"}
          >
            {sharingLocation ? "Sharing location" : recording ? "Recording" : "Secure mode"}
          </Badge>
        </div>
